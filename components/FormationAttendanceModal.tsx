...
export const FormationAttendanceModal: React.FC<FormationAttendanceModalProps> = ({ 
  ...
  const [presentIds, setPresentIds] = useState<string[]>(formation.presentes || []);
      ...
      <div className="bg-white w-full max-w-2xl rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[98vh] md:max-h-[85vh]">
        ...
        <div className="bg-slate-900 p-4 md:p-8 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h2 className="text-base md:text-xl font-black text-white leading-tight">Lista de Presença</h2>
              <p className="text-[9px] md:text-xs text-slate-400 font-bold uppercase tracking-widest truncate max-w-[150px] md:max-w-none">{formation.tema}</p>
            </div>
          </div>
        ...
        <div className="p-3 md:p-6 bg-slate-50 border-b border-slate-100 flex flex-col sm:flex-row gap-3 md:gap-4 shrink-0">
          <div className="flex-1 relative">
            ...
            <input 
              ...
              className="w-full pl-9 md:pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs md:text-sm outline-none focus:ring-2 focus:ring-green-500"
            />
          </div>
          <div className="bg-green-600 px-4 md:px-6 py-2 rounded-xl flex items-center justify-center gap-2 md:gap-3 text-white shadow-lg shadow-green-100">
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 md:p-4 rounded-xl md:rounded-2xl border transition-all ${
                    isPresent ? 'bg-green-50 border-green-200 shadow-sm' : 'bg-white border-slate-100 hover:border-slate-200'
                  }`}
                >
                  <div className="flex items-center gap-3 md:gap-4 min-w-0">
                    ...
                    <div className="text-left min-w-0">
                      <p className={`font-bold text-xs md:text-base transition-colors truncate ${isPresent ? 'text-green-900' : 'text-slate-800'}`}>{catequista.nome}</p>
                  ...
                  {isPresent ? (
                    <div className="bg-green-600 text-white p-1 rounded-full shadow-lg flex-shrink-0"><CheckCircle2 className="w-4 h-4 md:w-5 md:h-5" /></div>
                  ) : (
