      ...
      <div className="bg-white w-full max-w-2xl rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[98vh] md:max-h-[85vh]">
        <div className="bg-slate-900 p-4 md:p-8 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3 md:gap-4">
            <div className="bg-amber-600 p-2 md:p-3 rounded-xl md:rounded-2xl shadow-lg">
            ...
            <div>
              ...
              <p className="text-[9px] md:text-xs text-slate-400 font-bold uppercase tracking-widest truncate max-w-[150px] md:max-w-none">{catequista.nome}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-2 md:space-y-3 custom-scrollbar">
          {filteredDocs.length > 0 ? (
            filteredDocs.map(doc => (
              <div key={doc.id} className="flex items-center justify-between p-3 md:p-4 bg-white rounded-xl md:rounded-2xl border border-slate-100 hover:border-amber-200 transition-all group shadow-sm">
                <div className="flex items-center gap-3 md:gap-4 truncate">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:text-amber-500 transition-colors shrink-0">
                    <File size={20} className="md:hidden" />
                    <File size={24} className="hidden md:block" />
                  </div>
                  <div className="truncate">
                    <p className="font-bold text-slate-800 text-xs md:text-sm truncate">{doc.nome}</p>
                    <p className="text-[8px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 md:mt-1">
                ...
                <div className="flex items-center gap-0.5 md:gap-1 shrink-0">
                  <button 
                    ...
                    className="p-2 md:p-2.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg md:rounded-xl transition-all"
                    title="Visualizar"
                  >
            ...
            <div className="py-10 md:py-20 text-center bg-slate-50/50 rounded-2xl md:rounded-3xl border-2 border-dashed border-slate-200">
               <FileText className="w-10 h-10 md:w-12 md:h-12 mx-auto mb-3 md:mb-4 text-slate-200" />
               <p className="text-[10px] md:text-sm font-bold text-slate-400 uppercase tracking-widest">Nenhum documento anexado</p>
               <button 
                 onClick={() => fileInputRef.current?.click()}
                 className="mt-3 md:mt-4 text-amber-600 text-[9px] md:text-xs font-black uppercase tracking-widest hover:underline"
               >
        ...
        <div className="p-4 md:p-8 border-t border-slate-100 bg-slate-50 flex justify-end shrink-0">
          <button 
            onClick={onClose}
            className="w-full md:w-auto px-10 py-3 bg-slate-900 text-white font-black rounded-xl md:rounded-2xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 uppercase tracking-widest text-[10px]"
          >
            Fechar
          </button>
        </div>
